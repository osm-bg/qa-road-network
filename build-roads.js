...
function save_data(routes) {
    ...
    if(!fs.existsSync('output/road-routes')) {
        fs.mkdirSync('output/road-routes');
    }

    for(const route of routes.values()) {
        const filename = `output/road-routes/route-${route.ref.toString().replace(/\s+/g, '_')}.json`;
...

    {
        ...
        fs.writeFileSync('output/road-routes/routes.json', JSON.stringify(to_save, null, 2));
    }

    {
        ...
        fs.writeFileSync('output/road-routes/routes-map.js', rows.join('\n') + '\n');
    }
...
function run() {
    ...
    fetch_data([
        [
            ...
            { key: 'network', value: 'bg:motorway' }
        ],
        [
            { key: 'type', value: 'route' },
            { key: 'route', value: 'road' },
            { key: 'network', value: 'bg:national' }
        ],
        [
            { key: 'type', value: 'route' },
            { key: 'route', value: 'road' },
            { key: 'network', value: 'bg:municipal' }
        ]
    ])
