...
function save_data(routes) {
    ...
    if(!fs.existsSync('output/railway-routes')) {
        fs.mkdirSync('output/railway-routes');
    }

    for(const route of routes.values()) {
        const filename = `output/railway-routes/route-${route.ref.toString().replace(/\s+/g, '_')}.json`;
...

    {
        ...
        fs.writeFileSync('output/railway-routes/routes.json', JSON.stringify(to_save, null, 2));
    }

    {
        ...
        fs.writeFileSync('output/railway-routes/routes-map.js', rows.join('\n') + '\n');
    }
